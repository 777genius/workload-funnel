...
export interface SqliteCapacityLedgerState {
...
export function createSqliteCapacityReservationLedgerStore(
  state: SqliteCapacityLedgerState,
  ...
  const store: OwnerSafeCapacityReservationLedgerStore = {
    ...
    reserve(input: ReserveAllocationInput) {
      ...
      const value: Allocation = Object.freeze({
        allocationId: `sqlite-allocation-${String(++state.sequence)}`,
